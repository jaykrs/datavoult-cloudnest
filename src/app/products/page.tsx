...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    fetch(url).then(r => r.json()).then(d => { setProducts(d.data?.products || []); setLoading(false); });
  }, [activeCategory]);
          ...
          <div style={{ padding: '48px 0 40px', background: 'var(--bg-surface)', borderBottom: '1px solid var(--border)' }}>
            <div className="container">
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 28, flexWrap: 'wrap' }}>
                {categories.map(c => (
                  <button key={c.value} onClick={() => setActiveCategory(c.value)} className="btn btn-sm" style={{ background: activeCategory === c.value ? 'var(--accent)' : 'var(--bg-elevated)', color: activeCategory === c.value ? 'white' : 'var(--text-secondary)', border: activeCategory === c.value ? 'none' : '1px solid var(--border)' }}>
                    {c.icon} {c.label}
                  </button>
                ))}
              </div>
                  ...
                  return (
                    <div key={product.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', overflow: 'hidden' }}>
                      <div style={{ padding: '32px 32px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
                        ...
                        <Link href={`/products/${product.slug}`} className="btn btn-secondary btn-sm">Details <ArrowRight size={14} /></Link>
                      </div>
                      ...
                      <div style={{ padding: 32, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                        {product.plans.map(plan => (
                          <div key={plan.id} className="plan-card" style={{ background: plan.isPopular ? `linear-gradient(135deg, ${meta.glow}, transparent)` : 'var(--bg-elevated)', border: plan.isPopular ? `1px solid ${meta.color}44` : '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20, position: 'relative' }}>
                            {plan.isPopular && (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 16 }}>
                              {Object.entries(plan.limits).slice(0, 4).map(([k, v]) => (
                                <div key={k} style={{ fontSize: 12, color: 'var(--text-muted)', display: 'flex', justifyContent: 'space-between' }}>
                                  <span>{k}</span><span style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>{String(v)}</span>
                                </div>
                              ))}
                            </div>
                            <Link href={`/checkout?plan=${plan.id}&product=${product.id}`} className="btn btn-sm" style={{ width: '100%', justifyContent: 'center', display: 'flex', background: plan.isPopular ? meta.color : 'var(--bg-hover)', color: plan.isPopular ? 'white' : 'var(--text-secondary)', border: plan.isPopular ? 'none' : '1px solid var(--border)' }}>
                              Subscribe
                            </Link>
                          </div>
                        ))}
